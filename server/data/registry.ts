...
export const ENTITIES: Entity[] = [
  ...
  // ---- talent acquisition
  { key: "requisitions", table: s.requisitions, tableName: "requisitions", labelAr: "الاحتياجات الوظيفية", labelEn: "Requisitions", group: "الاستقطاب", sourceAr: "إدخال يدوي", labelField: "roleAr", module: "talent", columns: [
    col("code", "الرمز", "text", { readOnly: true }), col("roleAr", "المسمى الوظيفي", "text", { required: true }), col("sectorId", "القطاع", "fk", { fk: "sectors", required: true }), col("projectId", "المبادرة", "fk", { fk: "projects" }), col("engagementType", "نوع الاستقطاب", "select", { options: s.ENGAGEMENT_TYPES, required: true }),
    col("band", "الفئة", "select", { options: s.BANDS }), col("count", "العدد المطلوب", "number"), col("filled", "المشغول", "number", { readOnly: true }), col("priority", "الأولوية", "select", { options: ["عاجلة", "مرتفعة", "متوسطة"] }), col("isSenior", "وظيفة قيادية", "boolean"), col("targetStart", "تاريخ المباشرة المستهدف", "date"), col("status", "الحالة", "select", { options: ["مفتوح", "مكتمل", "ملغى"] }) ] },
  { key: "candidates", table: s.candidates, tableName: "candidates", labelAr: "المرشحون", labelEn: "Candidates", group: "الاستقطاب", sourceAr: "Odoo HR Recruitment / يدوي", labelField: "nameAr", module: "talent", columns: [
    col("code", "الرمز", "text", { readOnly: true }), col("nameAr", "الاسم", "text", { required: true }), col("requisitionId", "الاحتياج", "fk", { fk: "requisitions", required: true }), col("engagementType", "النوع", "select", { options: s.ENGAGEMENT_TYPES, readOnly: true }), col("sourceAr", "المصدر / الجهة", "text"),
    col("currentRoleAr", "الوظيفة الحالية", "text"), col("clearanceStatus", "الفحص الأمني", "select", { options: s.CLEARANCE }), col("monthlyRate", "الأجر الشهري (ألف)", "number"), col("secondmentMonths", "مدة الإعارة (شهر)", "number"), col("referenceAr", "المرجع", "text"), col("status", "الحالة", "select", { options: ["قيد الإجراء", "مباشر", "مستبعد"], readOnly: true }) ] },
  // ---- innovation
  { key: "innovationAssessments", table: s.innovationAssessments, tableName: "innovation_assessments", labelAr: "تقييمات نضج الابتكار", labelEn: "Innovation Assessments", group: "الابتكار", sourceAr: "تقييم ذاتي مُراجع", labelField: "cycle", module: "innovation", columns: [
    col("cycle", "الدورة", "text", { required: true }), col("subjectType", "نوع الجهة", "select", { options: ["sector", "region"], required: true }), col("subjectId", "معرّف الجهة", "number", { required: true }), col("overall", "النضج العام", "number", { readOnly: true }), col("level", "المستوى", "number", { readOnly: true }), col("assessorAr", "المقيّم", "text"), col("evidenceAr", "الشواهد", "text"), col("status", "الحالة", "select", { options: ["مسودة", "منشور"] }), col("assessedAt", "تاريخ التقييم", "date") ] },
  { key: "innovationIdeas", table: s.innovationIdeas, tableName: "innovation_ideas", labelAr: "الأفكار الابتكارية", labelEn: "Innovation Ideas", group: "الابتكار", sourceAr: "منصة الأفكار / يدوي", labelField: "titleAr", module: "innovation", columns: [
    col("code", "الرمز", "text", { readOnly: true }), col("titleAr", "الفكرة", "text", { required: true }), col("category", "التصنيف", "select", { options: s.IDEA_CATEGORIES }), col("sourceType", "نوع المصدر", "select", { options: ["sector", "region"] }), col("sourceId", "معرّف المصدر", "number"), col("submittedByAr", "مقدّم الفكرة", "text"), col("submittedAt", "تاريخ التقديم", "date"), col("impactValue", "الأثر التقديري (مليون/سنة)", "number"), col("linkedProjectId", "المبادرة المرتبطة", "fk", { fk: "projects" }), col("status", "الحالة", "select", { options: ["قيد الإجراء", "موسّعة", "مستبعدة"], readOnly: true }) ] },
